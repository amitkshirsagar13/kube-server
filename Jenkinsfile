...
volumes: [
  ...
  node(label) {
    ...
    stage('Deploy helm release') {
      ...
      container('helm') {
    	sh "helm upgrade --install $application --namespace dev ./cicd/kube-server/ --set profile=$profile --set branch=$gitBranch --set commit=$shortGitCommit --set application=$application"
      }
    }
  }
}
